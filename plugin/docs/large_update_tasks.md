...
1. **5-4h エラー監視** — 任意（Sentry or Cloudflare Workers Logs + Slack 通知）。ユーザー数が増えてから入れるで可
2. **5-4m Supabase Custom Auth Domain** — Pro プラン移行時の対応（Google 同意画面の `supabase.co` 表示を `leadace.ai` に差し替える。ユーザー数拡大後）
3. **旧 `aitit-inc/claude-plugins` GitHub Archive 指定**（任意、後日）
...
### 5-4o. Cookie 利用同意バナー ✅ 完了

**監査結果（実装前の現状把握）:**
- LP (`leadace.ai`): 独自 cookie / localStorage ゼロ。Google Fonts のみ外部読み込み（IP が Google に渡る）。Analytics / tracker ゼロ
- App (`app.leadace.ai`): Supabase Auth JWT が localStorage（`sb-*-auth-token`）、`leadace.theme`、`leadace_active_project`、いずれもユーザー明示操作の結果。Analytics / tracker ゼロ
- Stripe Checkout / Google OAuth は遷移先で独自 cookie を設定するが、戻り時に独自トラッキングなし

**法的結論:**
GDPR / ePrivacy 的に「strictly necessary」のみなので consent バナーは法的には不要。ただし透明性義務 + EU 流入時の信頼性のため、以下 2 点を実装（案 B 採用）。

**実装済み:**
- [x] `frontend/src/lib/components/CookieBanner.svelte` 作成。localStorage `leadace.cookie_consent=accepted` で dismiss 記憶。root `+layout.svelte` に埋め込んで全ページ（login / terms / privacy / 認証後）に表示
- [x] `landing/public/index.html` にバナー CSS + JS inline 追加。同一キー `leadace.cookie_consent` で独立管理（サブドメイン間で localStorage 共有不可のため別表示・別 dismiss）
- [x] `/privacy` §6 を詳細化（Strictly necessary / Preferences / Third parties の 3 区分。`id="cookies"` アンカー追加でバナーから直リンク）
- [x] メッセージ: "We use first-party cookies and local storage for authentication and to remember your preferences. We don't use analytics or advertising trackers. [Learn more]."

**将来対応（analytics 追加時）:**
- 案 C（consent-gated loader）に拡張: 現バナーを "Accept all / Reject all" の 2 ボタンに変更 → `localStorage.setItem('leadace.consent', 'analytics|none')` → analytics スクリプトは consent=analytics 時のみ動的ロード。現状バナー UI が単純な dismiss のため、この拡張は後日
...
- [x] 新インストール手順は `plugin/README.md` / root `README.md` で整理済み（`aitit-inc/lead-ace`）
- [x] 旧 `aitit-inc/claude-plugins` リポから lead-ace 関連を削除（`plugins/lead-ace/` + marketplace.json の plugin エントリ + README/CLAUDE.md の lead-ace 記述）。commit: `chore: remove lead-ace plugin (moved to aitit-inc/lead-ace)`
- [x] プラグイン .claude-plugin/plugin.json のバージョンアップ（0.5.23 — ブランド統一コミット時）

### 省略した理由

- **既存ユーザーのデータ移行スクリプト不要**: 旧 SQLite 時代のユーザーは極少数、かつ本プロダクト自体が MVP 公開前のため。新ユーザーはそのままクラウド版を使う
- **移行案内 README 不要**: 旧リポに「引越しのお知らせ」を残すと検索流入時に紛らわしい。旧リポは単に lead-ace 痕跡を消して放置する方針

### 保留（任意）

- 旧 `aitit-inc/claude-plugins` リポ自体を GitHub 側で Archive 指定するか、空のマーケットプレイスとして残すかは後日判断
...
フェーズ5-4（MVP 公開前の仕上げ）
    ...
    - 5-4n ブランド名を "LeadAce" に統一 ✅ 完了（コミット 2b74858、OG PNG 再生成含む）
    - 5-4o Cookie 利用同意バナー ✅ 完了（LP + app 両方、案 B 採用 — strict necessary のみなので dismiss 型）
...
フェーズ5-2（セルフデプロイ対応・ドキュメント）✅ コード完了
    ...
    ↓
フェーズ6（リリース・移行）✅ 完了
    - 旧 aitit-inc/claude-plugins から lead-ace 削除 ✅
    ↓
★ 現在地 ★ = 完全 MVP 公開可能状態
    - 任意: 5-4h エラー監視（Sentry 等）
    - 後日: 5-4m Supabase Custom Auth Domain（Pro プラン移行時）
    - 後日: 旧 aitit-inc/claude-plugins リポ自体の GitHub Archive 指定（任意）
    ↓
