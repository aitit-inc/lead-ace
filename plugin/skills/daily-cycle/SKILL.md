...
Verify that the `$0` directory exists and the project is registered on the server:
...
Write build-list summary (added count, reachable count, etc.) to `$0/.tmp/build-list-summary.md`.
...
```bash
git add "$0/" && git commit -m "work: :e-mail: $0" && git push
```
