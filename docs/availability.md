...
「ローカル CLI ツール」とは `gog`（メール送信）、`playwright-cli`（フォーム送信）、
`fetch_url.py`（Web 取得）、`claude-in-chrome` MCP（SNS DM）、Gmail MCP（返信確認）
など、ホスト OS 上の補助ツールを指す。これらが使えない環境では、メール送信・フォーム
送信・SNS DM・受信確認といったアウトバウンド系機能が事実上利用不可になる。
...
### ローカル依存
- `gog`（メール送信）— `/outbound`, `/daily-cycle` で使用
- `playwright-cli`（フォーム送信）— `/outbound` で使用
- `claude-in-chrome` MCP（SNS DM）— `/outbound`, `/check-results`
- Gmail MCP（返信確認）— `/check-results`
...
- **ローカル CLI ツール（`gog`, `playwright-cli`, `fetch_url.py`）が Cowork
  デスクトップ環境で実行できるかは公式に明記なし**。Cowork が Claude Code 同等の
  Bash 実行モデルを採用していれば動作する可能性が高いが、要実機検証。
...
ローカルの `gog` や `playwright-cli` は呼び出せない。LeadAce のうち「メール送信」
「フォーム送信」「SNS DM」「Web ページ取得」は事実上利用不可。
...
### ローカル CLI ツール
**Yes.** Codex はローカル shell 実行が可能なので、`gog`, `playwright-cli`,
`fetch_url.py` は問題なく利用可能。
